...
int exibeLigaEspanha(listaDupla *lista, listaDupla *lista2,SemanaBrasil* campeonato,int rodada,int rodadaOficial,int outrasliga,playoffsConfrontos *jogo,int liga){
	...
	do{
		...
		textcolor(15);
		MolduraColorida(1, 1, 120, 30,4,4,14,14);Moldura(10, 5, 68, 20);	
		gotoxy(12,4);printf("TABELA GERAL");
		textcolor(4);gotoxy(25,4);printf("KL - PENALTY ESPANHA");textcolor(15);
		...
		switch(op){
			case 'W':
    			...
    			do{
    				if(passarPagina1 == 'A'){
    					passarPagina1 = exibirEstatisticas(lista,2);
    				}
    				if(passarPagina1 == 'G')
    				{
    					passarPagina1 = exibirTopGoleiros(lista,2);
    				}
    				if(passarPagina1 == 'P'){
    					passarPagina1 = exibirTopPresidentes(lista,2);
    				}
			...
			case 'P':
				if(liga == 1){
					if(rodadaOficial>9){
						clrscr();
						exibirPlayoffsEspanha(rodadaOficial, lista,jogo,(rodadaOficial + 4) - 11);
					}
					if(rodadaOficial < 9){
					...
					if(rodadaOficial < 12){
			    		...
			    		clrscr();
			    		exibirPlayoffsEspanha(rodadaOficial, lista,jogo,rodadaOficial - 11);
			    	}
				}   
			break;
		}
